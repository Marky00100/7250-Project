...
```{r LMI Job Data Loading for State}
...
filtered_data <- master_aggregated_data %>% #which is ONLY BACHELOR EARNERS
  ...
  left_join(cips, by = c("cip_code" = "CIP2020Code")) %>%
      left_join(socs, by = c(  "soc_code" = "SOC2018Code"))%>%
 mutate(graduates_ploted = round((total_graduates), 2),  
  log_yearly_postings = round(ifelse(annual_openings_growth > 0, log(annual_openings_growth + 1), NA_real_), 2
),  # Fix for NA's
    cip_group = substr(cip_code, 1, 2)  # Extract two-digit CIP group
  ) %>% 
  filter(!is.na(CIP2020Title))%>%
filter(!is.na(log_yearly_postings))%>%   # Remove titles that did not make it into 2020
  filter(graduates_ploted > 10)

# Deduplicate the data before selecting observations
deduplicated_data <- filtered_data %>%
  select(CIP2020Title, SOC2018Title, median_wage, graduates_ploted, log_yearly_postings, cip_group, soc_code, jobsohioregion) %>%
...
top_left_observations <- deduplicated_data %>%
  group_by(cip_group) %>%
  arrange(cip_group, graduates_ploted, desc(log_yearly_postings)) %>%  # Sort by minimum graduates and maximum postings
  ...
  select(cip_group, CIP2020Title, SOC2018Title, graduates_ploted, log_yearly_postings, median_wage)

# Print the final table with  smaller text size and limit on decimal places
kable(
  top_left_observations, 
  format = "latex", 
  caption = "Top-left Observations for Each CIP Group", 
  digits = 2
) %>%
  kable_styling(latex_options = "scale_down", font_size = 7)

# Create Dot Plot for Each CIP Group, sdave this as a function first, then run for each group
create_dotplot <- function(data) {
  ...
  ggplot(data, aes(x = graduates_ploted, y = log_yearly_postings, color = gap_ratio, size = median_wage)) +
    ...
    labs(
      title = paste("CIP Group:", unique(data$cip_group)),
      x = "Graduates (with jitter)",
      y = "Log Yearly Postings",
      color = "Gap Ratio",
      size = "Median Wage",
...
# Generate and display dot plots for each CIP group
plots <- filtered_data %>%
  group_by(cip_group) %>%
  do(plot = create_dotplot(.))
